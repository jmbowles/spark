...
pipeline = Pipeline(stages=[tokenizer, sw_remover, count_v, lda])
pipeline_model = pipeline.fit(texts)
pipeline_df = pipeline_model.transform(texts)

lda_model = pipeline_model.stages[3]
vector_model = pipeline_model.stages[2]

topics = lda_model.describeTopics(maxTermsPerTopic=10)
topics = topics.withColumn('topic_words', indices_to_terms(pipeline_model.stages[2].vocabulary)('termIndices'))
print('The topics described by their top-weighted terms:')
topics.show()

ll = lda_model.logLikelihood(vector_model)
#lp = lda_model.logPerplexity(vector_model)
...
results = pipeline_df.withColumn('topic_index', topic_index()('topicDistribution'))
results.show()
